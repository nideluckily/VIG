...
int Evolve_WYT(vector<int>& BestSeq, int best_span, vector<int>& Seq1, int Span1, vector<int>& Seq2, int Span2, int d, int TimeLimit, int Temperature)
{
	...
	{
		//Seq1 进化
		//temp_span1 = DR_WYT(temp_seq1, d); //破坏，跳跃重构
		temp_span1 = DR_normal(temp_seq1, d, 0); //破坏顺序重构
		//temp_span1 = DR_Jump(temp_seq1, d, len, 0);
		...
		//temp_span1 = GreedyInsert(temp_seq1, temp_span1);
		...
		//Seq2  进化
		//temp_span2 = DR_WYT(temp_seq2, d);
		temp_span2 = DR_normal(temp_seq2, d, 1);
		//temp_span2 = DR_Jump(temp_seq2, d, len, 1);
		...
		//temp_span2 = GreedyInsert(temp_seq2, temp_span2);
